...
function getProjectDirectory({
  ...
  return _path().default.relative(process.cwd(), directory || projectName);
}
...
async function setProjectDirectory(directory) {
  ...
  if (directoryExists) {
    ...
    await _fsExtra().default.emptyDir(directory);
  }
...
var initialize = async function initialize([projectName], context, options) {
  ...
  try {
    ...
    const projectFolder = _path().default.join(rootFolder, projectName);

    (0, _printRunInstructions.default)(projectFolder, projectName);
  } catch (e) {
    _cliTools().logger.error(e.message); // Only remove project if it didn't exist before running `init`


    if (!directoryExists) {
      _fsExtra().default.removeSync(_path().default.resolve(rootFolder, directoryName));
    }
  }
};
